 ...
 *
 * 零依赖：从 `apps/web-agent/src/lib/subagent-card.ts` 迁入 web-common（Task 6，
 * 迁移时逻辑与测试逐行未改）——原文件已是 node 环境可直接加载测试的纯函数集，
 * 天然满足 web-common 的 jotai / next-intl / app 路径禁令，故整体迁移而非拆分。
 * web-agent 侧改为从本模块 re-export，`@/lib/subagent-card` 既有 import 路径不变；
