...
export interface AskQuestionCardProps {
  tool: ToolCallView;
  /**
   * 提交回答。HITL 收敛点（Task 5 裁定）：本地/远程会话统一走
   * `useSessionStream().answer`（内部已按 SessionTransport 路由），本组件
...
export function AskQuestionCard({
  ...
  const submit = async () => {
    ...
    try {
      await onAnswer(tool.toolCallId, answers);
    } catch {
      setBusy(false);
    }
  };
  ...
  return (
    <div className="flex w-full flex-col gap-3 rounded-lg border border-border bg-muted/30 px-3 py-2">
      {questions.map((q, qi) => (
