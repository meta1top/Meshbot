/**
 * 内置工具展示纯逻辑（友好名映射 + 路径打码）。
 *
 * 从 `apps/web-agent/src/lib/tool-display.ts` 迁入（Task 8）——零 next-intl/
 * jotai 依赖，原文件全部是字面量中文映射，无 t() 调用，整体搬迁行为不变；
 * 原文件改为 re-export，`@/lib/tool-display` 既有 import 路径不变。
 */
