...
export interface MessageListProps {
  ...
  /**
   * 当前登录用户展示名（已解析好含 fallback，原 `currentUserAtom` 读取 +
   * `t("youName")` 兜底逻辑由调用方负责）。仅 `!nested` 时渲染，nested 场景
   * 可不传。
   */
  userName?: string;
  assistantName?: string;
  /** AssistantMessageActions 用量 tooltip 解析模型友好名的配置列表；`!nested && !readOnly` 时才用得到。 */
  modelConfigs?: ModelConfigLike[];
  /** AssistantMessageActions 反馈提交回调；`!nested && !readOnly` 时才用得到。 */
  onFeedback?: (
    sessionId: string,
    messageId: string,
    value: "up" | "down" | null,
  ) => Promise<unknown>;
  /** AssistantMessageActions 文案；`!nested && !readOnly` 时才用得到。 */
  assistantActionsLabels?: AssistantMessageActionsLabels;
  /** UserMessageActions 重新生成回调；`!nested && !readOnly` 时才用得到。 */
  onRegenerate?: (sessionId: string, messageId: string) => Promise<unknown>;
  /** UserMessageActions 失败时的上抛（可选）。 */
  onActionsError?: (err: unknown) => void;
  /**
   * im_send_message 卡片的会话目标展示名解析：调用方按 conversationId 查本地
   * IM 会话列表，返回展示名（转发给 ToolCallBlock）。
   */
  resolveImTargetName: (conversationId: string | undefined) => string;
  /** present_file 卡片点击预览（转发给 ToolCallBlock）。 */
  onPreviewArtifact: (target: ArtifactPreviewTarget) => void;
  /** 当前会话所在的远程设备信息（本地会话为 null/undefined），转发给 ToolCallBlock。 */
  artifactRemote?: { deviceId: string; sessionId: string } | null;
  /** dispatch_subagent 嵌套卡渲染插槽，转发给 ToolCallBlock（见 ToolCallBlockProps JSDoc）。 */
  renderSubagentCard: (tool: ToolCallView) => React.ReactNode;
  /** 转发给 ToolCallBlock 自身的文案。 */
  toolCallLabels: ToolCallBlockLabels;
  /** 本组件自身渲染用到的文案。 */
 ...
 *
 * 从 `apps/web-agent/src/components/session/message-list.tsx` 迁入
 * （Task 9 骨干批）。`useTranslations`/`currentUserAtom` 改 `labels`/
 * `userName`/`assistantName` props；原先经由 web-agent 侧薄容器
 * （`./assistant-message-actions`/`./user-message-actions`/`./tool-call-block`）
          ...
          return (
            ...
            >
              ...
              */}
                ...
                {!nested && !readOnly && m.role === "user" && onRegenerate && (
                  <UserMessageActions
                    sessionId={sessionId}
                    messageId={m.id}
                    content={m.content}
...
function ReasoningBlock({
  ...
  const label = isThinking
    ? elapsed > 0
      ? labels.reasoningThinking((elapsed / 1000).toFixed(1))
      : labels.reasoningThinking("0.0")
    : elapsed > 0
      ? labels.reasoningThought((elapsed / 1000).toFixed(1))
      : labels.reasoningProcess;
  return (
    <div className="flex flex-col gap-2">
      <button
        type="button"
