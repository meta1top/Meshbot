 ...
 * 而 `agentId` 是父组件受控的 prop（`agent-rail` 只会传 null 或某个既有
 * agent id，不知道复制诞生的新 id）——用 `localAgentId` 内部状态承接：
 * 每次抽屉从关到开都用 prop 重置一次，但打开期间可以被 `handleDuplicate`
 * 改写，脱离 prop 独立驱动 UI，同时把 `currentAgentIdAtom` 也切过去,
 * 让导航条与抽屉的「当前 agent」认知保持一致。
 */
