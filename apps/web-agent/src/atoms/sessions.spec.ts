...
describe("renameSessionAtom：updatedAt 取服务端值（review Minor 1）", () => {
  it("成功路径取服务端 updatedAt —— 否则刷新后列表位置会突然跳变", async () => {
    const store = createStore();
    store.set(sessionsAtom, [
      makeSession("old", "idle", "2026-07-18T00:00:00.000Z"),
      makeSession("new", "idle", "2026-07-20T00:00:00.000Z"),
    ]);
    // 服务端 repo.update 会真实 bump updatedAt
    (patchSessionApi as jest.Mock).mockResolvedValueOnce({
      ...makeSession("old", "idle", "2026-07-21T12:00:00.000Z"),
      title: "改过了",
    });
    await store.set(renameSessionAtom, { id: "old", title: "改过了" });
    const rows = store.get(sessionsAtom);
    expect(rows[0].id).toBe("old"); // 已按新 updatedAt 排到最前
    expect(rows[0].updatedAt).toBe("2026-07-21T12:00:00.000Z");
  });

  it("回滚路径不动 updatedAt —— 那次写入我们并没有改过它", async () => {
    const store = createStore();
    store.set(sessionsAtom, [
      makeSession("s1", "idle", "2026-07-18T00:00:00.000Z"),
    ]);
    (patchSessionApi as jest.Mock).mockRejectedValueOnce(new Error("网络炸了"));
    await expect(
      store.set(renameSessionAtom, { id: "s1", title: "新标题" }),
    ).rejects.toThrow(/网络炸了/);
    expect(store.get(sessionsAtom)[0].updatedAt).toBe(
      "2026-07-18T00:00:00.000Z",
    );
    expect(store.get(sessionsAtom)[0].title).toBe("会话 s1");
  });
});
