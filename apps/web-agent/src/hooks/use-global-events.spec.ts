...
// window.alert → shadcn AlertDialog 承载改造：`globalAlertMessageAtom` 换成一个
// 可识别的哨兵值（而非 `{}`），供下方 "jotai" mock 的 `useSetAtom` 按身份区分出
// 这一个调用点，返回 `setGlobalAlertMessage`（其余 useSetAtom 调用仍返回匿名
// 空操作函数，测试不关心它们）。
jest.mock("@/atoms/global-alert", () => ({
  globalAlertMessageAtom: "GLOBAL_ALERT_ATOM",
}));
...
// 缺陷 1 引入：router.push("/assistant")，node 测试环境本无 window/next-intl/
// next-navigation，同样需要桩掉（见下方两个 jest.mock）。
jest.mock("next-intl", () => ({ useTranslations: jest.fn() }));
jest.mock("next/navigation", () => ({ useRouter: jest.fn() }));
// window.alert → shadcn AlertDialog 承载改造后引入的哨兵：`useSetAtom` 收到
// `globalAlertMessageAtom`（上面 mock 成字符串 "GLOBAL_ALERT_ATOM"）时返回这一个
// 固定 spy，测试直接断言它被调用了什么内容；其余 atom 身份不关心，一律退回
// 匿名空操作函数（保持原有行为）。
const setGlobalAlertMessage = jest.fn();
...
jest.mock("jotai", () => ({
  useSetAtom: jest.fn((atom: unknown) =>
    atom === "GLOBAL_ALERT_ATOM" ? setGlobalAlertMessage : jest.fn(),
  ),
 ...
 * 进了那个判定函数，并在命中时真的写 `globalAlertMessageAtom`
 * （`setGlobalAlertMessage`，原生 `window.alert` 的 shadcn AlertDialog 替代
 * 承载，见 `atoms/global-alert.ts` + `components/shell/global-alert-host.tsx`）
 * + `router.push`——即“本机路径”与“远程路径”都有测试真实走过
 * `useGlobalEvents` 的完整分发链路，不是只测了被抽出来的纯函数。
 */
describe("useGlobalEvents 缺陷 1 接线：当前会话被删除 → alert + 跳回起手台", () => {
  ...
  describe("本机路径（sessionsAtom + SESSION_LIFECYCLE_EVENTS.deleted）", () => {
    it("当前打开的本机会话被删除（非自删）→ alert + 跳回 /assistant", () => {
      ...
      expect(setGlobalAlertMessage).toHaveBeenCalledTimes(1);
      expect(setGlobalAlertMessage).toHaveBeenCalledWith(
        "sessionDeletedElsewhere",
      );
    ...
    it("删除的不是当前打开的会话 → 不提示、不跳转", () => {
      ...
      expect(setGlobalAlertMessage).not.toHaveBeenCalled();
