 ...
 *
 * 从 `apps/web-agent/src/hooks/use-chat-scroll.ts` 原样复制（零依赖纯 hook，
 * `SessionConversationView` 所需的 `topSentinelRef`/`stickToBottom`/
 * `onScrollToBottom` 三项都来自这里）——本次未提升进 `web-common/session`
 * （任务范围未要求，且 web-agent 侧暂无改动动机），留作后续若有第三个消费方
 * 再合并的候选，见任务报告 concerns。
 */
