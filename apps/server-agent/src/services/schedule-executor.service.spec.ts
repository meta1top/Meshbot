...
describe("ScheduleExecutor 账号运行时生命周期事件（登录注册 / 登出反注册）", () => {
  ...
  it("boot 幂等（崩溃回归）：runtime 先注册后，onApplicationBootstrap 全量装载不重复注册 / 抛错", async () => {
    const job = await seedCronJob({ cloudUserId: "u1", title: "boot-idem" });
    // 实测崩溃场景：账号运行时恢复（runtimeCreated）先于 boot 全量装载注册了该 job
    await executor.onRuntimeCreated({ cloudUserId: "u1" });
    expect(registry.getCronJobs().has(job.id)).toBe(true);
    const before = registry.getCronJobs().get(job.id);

    // onApplicationBootstrap 全量装载对已注册 job 必须跳过，
    // 不能重复 addCronJob 抛 DUPLICATE_SCHEDULER（多账号 boot 实测崩溃点）
    await expect(executor.onApplicationBootstrap()).resolves.toBeUndefined();
