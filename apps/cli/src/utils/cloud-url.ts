 ...
 * - 显式 env `MESHBOT_CLOUD_URL` 最高优先级（自托管 / staging）；
 * - 其次 CLI config 的 `cloudUrl`（`meshbot config set cloudUrl ...`）；
 * - monorepo 源码运行（pnpm dev:cli）→ 本地 3200；
 * - 分发安装版 → 生产 api-bot.meta1.top。
 */
export function resolveCloudUrl(opts?: {
  env?: NodeJS.ProcessEnv;
  cwd?: string;
  configCloudUrl?: string;
}): string {
  const env = opts?.env ?? process.env;
  if (env.MESHBOT_CLOUD_URL) return env.MESHBOT_CLOUD_URL;
  if (opts?.configCloudUrl) return opts.configCloudUrl;
