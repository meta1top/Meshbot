...
- **Track D（发布工具链）**：changesets 接入（`@meshbot/agent` + `@meshbot/server-agent` + `@meshbot/desktop` `fixed` 组共享版本）+ `release.yml`（changesets/action PR-driven 自动发版 + 补 `@meshbot/desktop@<v>` tag）；删 `publish-cli.yml`（npm publish 由 changesets 自动处理）；改 `package-desktop.yml` 触发器 + 上传 GitHub Release；CHANGELOG.md 索引 + 各包占位
