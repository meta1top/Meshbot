...
- **落地页各 section 一律 client component + `useTranslations`**，与 `LandingNav` 一致。**禁止服务端 next-intl 接线**（`getRequestConfig` / `createNextIntlPlugin` / `next-intl/server`）。原因（Task 3 审查实测）：① `intl-provider.tsx:27` 在 SSR 时 `document` 未定义、恒返回 `defaultLocale`，服务端组件读真 cookie 会与客户端组件分叉，`locale=en` 下首屏输出中英混排；② `request.ts` 里的 `await cookies()` 会让 `/` 从 `○ Static` 退化为 `ƒ Dynamic`，而 infra nginx 的 `location /` 无缓存层兜底；③ 收益为零——`intl-provider.tsx:18-19` 已把 zh.json 与 en.json 两份全量静态 import 进客户端 bundle。
...
创建 `landing-hero.tsx`，照 mockup 第 239–271 行移植。**是 client component**（顶部 `"use client"`，用 `useTranslations`），所有文案改为 `t()`。三栏工作空间全景的入场动画用 CSS `animation-delay`，延迟值与 mockup 一致：`.25s / 1.5s / 4.6s`（左栏消息）、`2.1s / 2.8s / 3.5s / 4.1s`（中栏工具行与产物）。
...
### Task 9: 双语上线（路径分离 `/` = zh，`/en` = en）

终审发现：分支里有约 220 行正式英文文案，但页面无语言切换入口、`<html lang="zh-CN">` 硬编码、
且 `locale=en` 下实测 hydration 失败（`intl-provider.tsx:27` 在 SSR 读不到 `document.cookie`，
恒返回默认中文）。用户拍板完整双语上线。

**方案：路径分离。** `/` 渲染中文、`/en` 渲染英文，两者均保持静态预渲染。
理由：cookie 单 URL 方案下爬虫只能拿到中文 SSR HTML（英文永不被索引），且英文页无法分享；
路径分离同时消除 SSR/hydration 分叉——每个路径的语言在构建期即确定。

**应用内路由（`(shell)` 等）保持现有 cookie 机制不变**，本 task 只处理落地页。

**Files:**
- Create: `apps/web-main/src/app/en/page.tsx`
- Modify: `apps/web-main/src/components/landing/landing-nav.tsx`（语言切换入口）
- Modify: `apps/web-main/src/app/page.tsx`（metadata 加 hreflang alternates）

**要求：**
1. `/en` 用 `NextIntlClientProvider` 显式注入 `locale="en"` 与 en messages，包住同一批 landing 组件——**不复制组件**，只包一层。
2. 语言切换入口放导航栏，用 `<Link>` 在 `/` 与 `/en` 间跳转，当前语言要有视觉标识。文案本身走 i18n。
3. `<html lang>` 必须与内容语言一致（WCAG 3.1.1）。**不得为此重构应用的 root layout**（影响全站风险过大）；可用路由段级方案或客户端同步，在报告里说明选型理由与局限。
4. 两个路径的 metadata 各自正确（title/description/OG 用对应语言），并互相声明 `alternates.languages` hreflang。
5. **两个路径构建后都必须是 `○ Static`**，且 `locale=en` cookie 下访问 `/` 不再产生 hydration 报错。

**验证：** 构建路由表贴报告（`/` 与 `/en` 均 `○`）；两个路径各自实机截图确认语言正确、`lang` 属性正确、切换往返可用；
`pnpm sync:locales -- --check` 仍 `missing=0, asymmetric=0`；控制台无 hydration 警告。
