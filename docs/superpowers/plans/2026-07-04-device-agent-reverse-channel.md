...
**★ 本任务必修:游标语义修正(T10 评审 Important,用户拍板)**——当前 `process`(Task 10)在 `finally` 无条件 `advanceCursor`,导致"run 成功、算出回复、但回流瞬间 relay 断连(relay.send 抛)"时回复被当失败丢弃、游标照常前进 → 计算好的回复永久丢失、补处理不重投。修正为**投递确认游标 + append 游标防 dup-append**:

- `im_agent_session` 加一列 `last_appended_message_id`(新 SQLite 迁移,时间戳 **1781000000000**,`ALTER TABLE ADD COLUMN`,down 保留);`ImAgentSessionService` 加 `advanceAppended(conversationId, messageId)` / `getAppended(conversationId)`。
- `process` 重构成"两段游标":
  1. **append 段**:`resolveSession` 里,若 `messageId <= getAppended`(已 append 过,幂等)则跳过 append 只取回 sessionId;否则 append + `advanceAppended(messageId)`。这防止回流失败后补处理重跑时把同一条 user 消息 **dup-append** 进本地 session。
  2. **run + 投递段**:`kickAndWait` → `findLastAssistant`;`runOk` 标记本次 run 是否成功。
  3. **游标(投递)段**:
     - run 成功 + `relay.send(reply)` 成功 → `advanceCursor(messageId)`(已投递);
     - run 成功 + `relay.send` **抛**(如 IM_NOT_CONNECTED)→ **不 advanceCursor**、不回错误消息、只日志("已计算未投递,待重连补投");留给 catchUp 重投(append 游标保证不 dup-append)。
     - run 失败 → best-effort `relay.send(错误消息)`;无论错误消息是否投递成功都 `advanceCursor`(避免同一条 run-崩消息无限重试)。
  - 即:**游标(last_processed)= 已投递;只在"投递成功"或"run 真失败"时前进;run 成功但投递失败不前进**。
- 相应更新 T10 遗留的两条 Minor 测试:失败分支断言 `relay.send` 被调;补 `findLastAssistant` 为 null → `NO_REPLY_TEXT` 兜底的用例。
