...
- [ ] **Step 1（TDD）**: service spec 先加三用例并跑红：
...
- [ ] **Step 2**: `org-model-config.service.ts` 的 create/update 接
...
- [ ] **Step 3**: `MODEL_SPECS` 清单扩充（key 精确匹配；数值实施时以官方文档核对）：
...
- [ ] **Step 4**: 跑绿 + 回归（`npx jest libs/main` 或该 spec 所在套件）+ commit
...
- [ ] **Step 1**: contextWindow `<Input>` 下加一排 chip 按钮：
...
- [ ] **Step 2**: `pnpm --filter @meshbot/web-main typecheck` + `pnpm sync:locales -- --write`
...
- [ ] **Step 4（sync 触发源修订）**: model-config-sync：
...
- [ ] **Step 6**: 回归：`npx jest apps/server-agent apps/server-main/src libs/main libs/common`
...
- [ ] **Step 1**: `TimelineMessage` 加 `errorText?: string`；`onError` 里对 failedIds
  命中的消息除 `failed:true` 外写 `errorText: e.error`（截断到合理长度如 200 字符）。
- [ ] **Step 2**: message-list 的 failed 分支下渲染错误行（小号 `text-destructive/80`，
  在消息正文下方、重试按钮语义不变）；仅 `errorText` 存在时渲染（历史恢复的 failed
  行无文案不渲染——可接受，spec 已记）。
- [ ] **Step 3**: i18n + typecheck + commit
