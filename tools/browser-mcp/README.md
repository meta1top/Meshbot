...
## 限速
navigate/click/type 每站每分钟默认 ≤30 动作（`BROWSER_MCP_MAX_ACTIONS_PER_MIN` 可调），超了自动按人类节奏冷却。

## v0.1 范围说明
- **登录态迁移用 `cookies_save/load`**（dump/灌 cookie），未实现 spec 设想的整包 `storage_state`（含 localStorage/sessionStorage）；主持久化靠每账号 `user_data_dir`，已够用，storage_state 留待后续。
- **`extract` 暂不落盘/截断**：大结果整段返回，海量评论可能撑大 agent 上下文；先克制 selector 范围，落盘分页留待后续。
