...
async login(@Body() dto: LoginDto, @Req() req: Request) {
  ...
  await this.sessionService.invalidatePreviousSessions(user.id);
...
async heartbeat(@CurrentUser() user: AppUser, @Param("id") id: string) {
  ...
  if (agent.ownerUserId !== user.id) {
    throw new AppError(ErrorCode.FORBIDDEN, "无权访问该 Agent");
  }
  return this.agentService.touchHeartbeat(agent.id);
}

// ✅ 正确：权限 / 存在性校验下沉到 Service（统一抛 i18n-key 业务错误）
@Post("agents/:id/heartbeat")
async heartbeat(@CurrentUser() user: AppUser, @Param("id") id: string) {
  return this.agentService.touchHeartbeatByOwner(id, user.id);
}
```
