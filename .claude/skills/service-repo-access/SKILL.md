...
grep -rn "@InjectRepository(FooEntity)" libs/
...
@Processor('queue')
export class FooProcessor extends WorkerHost {
  constructor(private readonly fooService: FooService) { super(); }
}
```
...
1. 在合适的 `libs/<domain>/src/.../entity/` 下创建 `*.entity.ts`
