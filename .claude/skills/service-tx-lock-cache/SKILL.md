...
`libs/common` 提供三个装饰器，**只能用在 Service 层方法上**（不可用于 Controller、Processor）。

> **meshbot 状态**：分布式锁 / 缓存底层由 `LockProvider` / `CacheProvider` 抽象，本地（desktop / server-agent）使用内存实现（async-mutex / lru-cache），云端（server-main）在 Phase 3 接入 Redis。下文行文沿用 "Redis" 表述时，理解为对应的 Provider 实现即可。

## @Cacheable / @CacheEvict

### 使用前提

Service 类必须标记 `@CacheableService()` 类装饰器，系统启动时自动注入 Redis 实例。
...
```ts
@Cacheable({ key: 'agent:instance:#{0}', ttl: 300 })
@Cacheable({ key: 'agent:tpl-ver:#{0}', ttl: 600 })
@Cacheable({ key: 'model:list:#{0}', ttl: 3600 })
@Cacheable({ key: 'session:#{0}', ttl: 60 })
```
...
| 数据特征 | TTL | 示例 |
|---------|-----|------|
| 极少变化（配置 / 模型列表） | 3600s | `ModelConfigService.listEnabled` |
| 低频变化（用户基本信息） | 300-600s | `UserService.findById` |
| 高频变化（session 状态） | 30-60s | `getSessionById` |
| 不可缓存 | 无 | 分页列表、实时计数 |
...
export class FooService {
  @Cacheable({ key: 'foo:#{0}', ttl: 300 })
  async findById(id: string) { ... }

  @CacheEvict({ key: 'foo:#{0}' })
  async update(id: string, data: Partial<FooEntity>) { ... }

  @CacheEvict({ keys: ['foo:#{0}', 'foo:related:#{0}'] })
...
```ts
@Cacheable({ key: 'model:name:#{0}', ttl: 3600, cacheNull: true })
...
```ts
@Transactional()
async createWithProvision(data) {
  const entity = await this.serviceB.create(data);   // serviceB 的 repo 自动在事务内
  await this.serviceC.bindResource(entity.id);        // serviceC 同理
  return entity;
}
```
...
@Module({
  imports: [TxTypeOrmModule.forFeature([FooEntity, BarEntity])],
...
BullMQ 投递、Redis 发布等外部副作用**不在事务回滚范围内**。将外部操作移到 `@Transactional` 方法返回后执行：

```ts
// ❌ 错误：BullMQ 投递在事务内，回滚时 job 已入队
@Transactional()
async createTask() {
  await this.repo.save(task);
  await this.queue.add('job', data);
  await this.repo.update(task.id, { jobId });
}

// ✅ 正确：拆分为事务方法 + 后续投递
@Transactional()
async createTaskInDb(params) {
  const task = await this.repo.save(this.repo.create(params));
  return task;
}

async createTask(params) {
  const task = await this.createTaskInDb(params);
  const jobId = await this.queue.add('job', { taskId: task.id });
  await this.repo.update(task.id, { bullSchedulerId: jobId });
  return task;
}
```
...
| 类型 | 形式 | 推荐场景 | 示例 |
|---|---|---|---|
| 后缀（首选） | `*InDb` | 单纯将"业务方法"中的 DB 写入步骤抽出 | `createTaskInDb`、`createOnceInDb` |
| 后缀 | `*InTx` | 同上，强调"在事务内" | `createOrderInTx` |
| 后缀 | `*InTransaction` | 同上，更全称写法 | `softDeleteInTransaction` |
| 前缀 | `persist[A-Z]*` | 跨多个子领域聚合落库（"持久化"语义） | `persistTenantWithOwner` |
...
少数情况下，命名带 `persist*` 的方法**故意不能放事务**——例如方法体内含 embedding HTTP / 外部 RPC。豁免方式：在 JSDoc 中加 `@no-tx-naming` 标记：

```ts
/**
 * 记忆提取批量落库（含 embedding HTTP，故意不放事务）
...
```ts
@WithLock({ key: 'agent:inst:name:#{0.userId}:#{0.name}', ttl: 10000 })
@WithLock({ key: 'agent:session:enqueue:#{0.sessionId}', ttl: 10000, waitTimeout: 5000 })
```
...
@Transactional()
async createOrder(userId: string, name: string) {
  await this.repo.save(order);
  await this.foo.checkAndCreateName(name);   // ← 内部带 @WithLock
}

@WithLock({ key: 'foo:name:#{0}' })
async checkAndCreateName(name: string) {
  const existed = await this.repo.findOne({ where: { name } });
...
```ts
async createOrder(userId: string, name: string) {
  await this.foo.checkAndCreateName(name);   // 自带锁 + 自带事务
  await this.createOrderInTx(userId, name);
}

@Transactional()
private async createOrderInTx(userId: string, name: string) {
  await this.repo.save(order);
}
```
...
```ts
@WithLock({ key: 'order:create:#{0}', ttl: 10000 })
@Transactional()
async createOrder(userId: string, name: string) {
  await this.repo.save(order);
  await this.foo.createName(name);   // 内部不要再带 @WithLock
}
```
